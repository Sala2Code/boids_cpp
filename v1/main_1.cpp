...
sf::VertexArray setSerf(int row=0, int col=0);
...
int degreCollision(sf::VertexArray serf, int num);
...
int main(){
    ...
    for(int i=0;i<nb_serf;i++){
        int mid_col = std::floor(config[1]/2);
        int mid_row = std::floor(config[0]/2);

        col = (i%config[1]>=config[1]/2 && config[1]/2.0f == mid_col ) ?  i%config[1] - mid_col + 1 : i%config[1] - mid_col ;
        row = (std::floor(i/config[1])>=config[0]/2 && config[0]/2.0f == mid_row ) ? std::floor(i/config[1]) - mid_row + 1 : std::floor(i/config[1]) - mid_row ;

        list_serf[i] = setSerf(row,col);
    }
...
sf::VertexArray setSerf(int row, int col){
    sf::VertexArray serviteur(sf::Triangles, 3);
    serviteur[0].position = sf::Vector2f(width/2 + col * taille*2, (height-taille)/2 + row * taille*2);
...
sf::VertexArray serf_update(sf::VertexArray serf, int num){
    ...
    if(marge>pos0.x || pos0.x>width-marge || pos0.y>height -marge|| 0>pos0.y-marge  ){    
        // Minimum entre 0,y et minimum entre width,y et minimum entre x,0 et x,height
        float angle_d = 15*PI/180;
        sf::Vector2f pos_plus = coord(centre[0],centre[1],serf[0].position.x,serf[0].position.y,degre+angle_d,pas,pos0_init); 
        sf::Vector2f pos_moins = coord(centre[0],centre[1],serf[0].position.x,serf[0].position.y,degre+angle_d,pas,pos0_init); 

        if (std::min(pos_plus.x,std::min(width-pos_plus.x,std::min(pos_plus.y,height-pos_plus.y))) < std::min(pos_plus.x,std::min(width-pos_plus.x,std::min(pos_plus.y,height-pos_plus.y)))){
            degre += - angle_d;
        }
        else{
            degre += angle_d;
...
float mean_neighbors(sf::VertexArray serf, int num, std::vector<float> centre){
    ...
    if(zone_collision > d_most_near){ //Eviter collision
        ...
        if(zone_align<d_most_near<zone_prox){
           return angleDeviate(angle, 2*PI/180, pos0_init, serf, false, centre, list_serf[nb_neighbor][0].position.x, list_serf[nb_neighbor][0].position.y);
        }
        else{
